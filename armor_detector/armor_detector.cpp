...
std::vector<Light> Detector::findLights(const cv::Mat &rgb_img,
    ...
    for (const auto &contour : contours) {
        ...
        if (isLight(light)) {
            // Determine light color based on RGB values
            int sum_r = 0, sum_b = 0;
            for (const auto &point : contour) {
                sum_r += rgb_img.at<cv::Vec3b>(point.y, point.x)[0];  // R channel
                sum_b += rgb_img.at<cv::Vec3b>(point.y, point.x)[2];  // B channel
            }
            
            int color_diff = std::abs(sum_r - sum_b) / static_cast<int>(contour.size());
            if (color_diff > light_params.color_diff_thresh) {
                light.color = sum_r > sum_b ? EnemyColor::RED : EnemyColor::BLUE;
            } else {
                light.color = EnemyColor::UNKNOWN;
            }
            
            // Only add lights that match the detection color or are unknown
            if (light.color == detect_color || light.color == EnemyColor::UNKNOWN) {
                lights.emplace_back(light);
            }
        }
    }
    
    // Sort lights by x-coordinate
...
bool Detector::isLight(const Light &light) noexcept {
    ...
    debug_lights.push_back(light_data);
...
std::vector<Armor> Detector::matchLights(const std::vector<Light> &lights) noexcept {
    ...
    for (auto light_1 = lights.begin(); light_1 != lights.end(); light_1++) {
        if (light_1->color != detect_color && light_1->color != EnemyColor::UNKNOWN) continue;
        
        double max_iter_width = light_1->length * armor_params.max_large_center_distance;

        for (auto light_2 = light_1 + 1; light_2 != lights.end(); light_2++) {
            if (light_2->color != detect_color && light_2->color != EnemyColor::UNKNOWN) continue;
...
bool Detector::containLight(const int i, const int j, const std::vector<Light> &lights) noexcept {
    ...
    for (int k = i + 1; k < j; k++) {
        ...
        // Prevent number interference
        if (test_light.width > 2 * avg_width) {
            continue;
        }
        
        // Prevent red dot sight or bullet interference
...
ArmorType Detector::isArmor(const Light &light_1, const Light &light_2) noexcept {
    ...
    debug_armors.push_back(armor_data);
...
void Detector::drawResults(cv::Mat &img) const noexcept {
    ...
    for (const auto &armor : armors_) {
        std::string text = fmt::format("{} {}", armorTypeToString(armor.type), armor.classification_result);
        cv::putText(
